    ...
    <div className={`flex h-full flex-col ${isFlipping ? "" : "bg-honey-tint"}`}>
      <header className="flex items-center justify-start px-4 pt-4 md:px-6 md:pt-6">
        {!isFlipping && (
          <button
            onClick={() => flipTo("/")}
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-full bg-paper px-3 py-2 text-sm font-semibold text-ink ring-1 ring-divider transition hover:bg-sage-tint"
          >
